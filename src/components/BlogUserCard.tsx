...
const BlogUserCards: FC<BlogUserCardsProps> = ({ blog, useInImg, useInName, useInSummary, useInRead, useInDate, useInCategory }) => {
    ...
    <div className="w-full md:w-full rounded-md overflow-hidden mt-2">
      <div className="w-full md:w-[500px] md:mr-4 border-slate-250 border-2 rounded-lg flex flex-col">
        ...
        <div className=" flex flex-col md:flex-grow md:h-auto ">
          {useInName && <h1 className="font-bold text-2xl mt-2 md:mt-5 ml-2 md:ml-5">
            {blog.name}
          </h1>}
          {useInSummary && <p className="text-normal mt-2 ml-2 md:ml-5">
            {blog.summary}
          </p>}
          {useInRead && <Link
            href={`/blogs/${blog._id}`}
          ...
          <div className="flex mt-5 ml-2 md:ml-5">
            {useInDate && <span className="text-black font-bold">
              21 October 2023
            </span>}
            {useInCategory && <span className="font-bold ml-40 md:ml-40">
              {blog.category}
            </span>}
          </div>
        </div>
